...
function build_new_post($type = 'thread', $foruminfo, $threadinfo, $postinfo, &$post, &$errors)
{
	...
	//$dataman->setr('currency_name', $post['currency_name']);
	...
	// $dupehash = md5($foruminfo['forumid'] . $post['title'] . $post['message'] . $vbulletin->userinfo['userid'] . $type); // original
    $text_mess = preg_replace('/\s+/', ' ', trim($post['message']));
    $dupehash = md5($foruminfo['forumid'] . $post['title'] . $text_mess. $vbulletin->userinfo['userid'] . $type);
	$prevpostfound = false;
	$prevpostthreadid = 0;
	...
	// ### DO INSTANT WEB PUSH NOTIFICATION ###
	if($threadinfo['threadid'])
	{
		...
		$gettups = $vbulletin->db->query("SELECT * FROM " . TABLE_PREFIX . " `subscribethread` as st
			LEFT JOIN " . TABLE_PREFIX . "`user` as us on us.userid = st.userid
			WHERE us.lastvisit < us.lastactivity AND st.emailupdate = 5 AND st.threadid = " . $threadinfo['threadid'] . " AND st.userid <> " . $vbulletin->userinfo['userid'] . ""
		);

		$sendcount = 0;
		while($gettup = $vbulletin->db->fetch_array($gettups))
		{
			$threadid = $gettup['threadid'];
			$userid = $gettup['userid'];
			$postid = $gettup['postid'];
			...
			$pnsmaxdateline = $vbulletin->db->query_first("SELECT MAX(dateline) as _max FROM " . TABLE_PREFIX ." push_notification_send
					WHERE userid = " . $userid . " AND threadid = " . $threadid ."
					AND sendstatus = 1");

			$stored = 0;
			if($pnsmaxdateline['_max'] != null)
			{
				$stored = $pnsmaxdateline['_max'];
			}

			$totaleffect = $vbulletin->db->query_read_slave("SELECT COUNT(*) AS total_effect
				FROM " . TABLE_PREFIX . "`user`
				LEFT " . TABLE_PREFIX . " JOIN `push_notification_send` AS pns ON user.`userid` = pns.userid
				WHERE user.`userid` = " . $userid . "
				AND " . intval($lastposttime['dateline']) ." <= user.lastactivity
				AND " . intval($lastposttime['dateline']) ." >= " . $stored . ""
			);

			while($totaleffect = $vbulletin->db->fetch_array($totaleffect))
			{
				// get last reply time
	/*			if ($postid)
				{
				...
				$ifexists = $vbulletin->db->query_first("SELECT COUNT(*) dataexist, dateline FROM " . TABLE_PREFIX . " `push_notification_send` 
					WHERE userid = " . $userid . " AND threadid = " . $threadid . " ");

				// $brid = $_COOKIE['os_uid'];
				// $brid = "be4284cf-4d18-4586-ad3d-692c7f90b66e";

				$brid = $vbulletin->db->query_read_slave("SELECT DISTINCT(pn.browserid) FROM " . TABLE_PREFIX . " `push_notification` as pn
					LEFT JOIN " . TABLE_PREFIX . " `user` as us ON us.userid = pn.userid WHERE pn.userid = $userid AND us.lastvisit <= pn.dateline");
				...
				$pagetext = $plaintext_parser->parse($pagetext_orig, $foruminfo['forumid']);
				...
				if($ifexists['dataexist'] != 0)
				{
					// checked equal?
					if($ifexists['dateline'] <= intval($lastposttime['dateline']))
					{
						$array = array ();
						$i = 0;
						$scount = 0;

						while ($brids = $vbulletin->db->fetch_array($brid)) {
							$obj = sendNotificationBySpecificUser($brids['browserid'], $threadlink, "$pagetext");
							$i++;
							$scount += intval($obj->recipients);
						}

						if($scount > 0)
						{
							$vbulletin->db->query_write("
							UPDATE " . TABLE_PREFIX . " `push_notification_send` SET dateline = " . intval($lastposttime['dateline']) . "
							WHERE userid = " . $userid . " AND threadid = " . $threadid . "
							");
				...
				{
					$vbulletin->db->query_write("
						INSERT INTO " . TABLE_PREFIX ." `push_notification_send`
						(
							userid,
							threadid,
							sendstatus,
						...
						(" .
							...
							$threadid . ", " .
							1 . ", " .
							intval($lastposttime['dateline']) . "
						)
					");

					sendNotificationBySpecificUser($brid['browserid'], $threadlink, "$pagetext");
				}
			}
		}
	}
...
/**
 * Sends Thread subscription Notifications for push notification
 *
 * @param	integer	The Thread ID
 * @param	integer	The User ID making the Post
 * @param	integer	The Post ID of the new post
 *
 */
function exec_send_notification_push()
{
	// $threadid = threadid to send from;
	// $userid = userid of who made the post
	// $postid = only sent if post is moderated -- used to get username correctly

	global $vbulletin, $postusername;
	...
	// get total subcribed thread
	$gettups = $vbulletin->db->query("SELECT td.threadid, us.userid, td.title FROM subscribethread AS st
		LEFT JOIN " . TABLE_PREFIX ."thread AS td ON st.threadid = td.threadid
		LEFT JOIN " . TABLE_PREFIX . "`user` AS us ON us.`userid` = st.userid
		WHERE us.userid = " . $vbulletin->userinfo['userid'] ."
		AND st.emailupdate = 5 AND st.canview = 1"
	);
	...
	{
		$threadid = $gettup['threadid'];
		$threadtitle = $gettup['title'];
		$userid = $gettup['userid'];
		$postid = $gettup['postid'];
		$threadlink = fetch_seo_url('thread|nosession', array('threadid' => $threadid, 'title' => htmlspecialchars_uni($threadtitle)), array('goto' => 'newpost'));
		// get last reply time
		if ($postid)
		{
			$dateline = $vbulletin->db->query_first("
		...
		// get the max dateline
		$pnsmaxdateline = $vbulletin->db->query_first("SELECT MAX(dateline) as _max FROM " . TABLE_PREFIX ." push_notification_send
				WHERE userid = " . $vbulletin->userinfo['userid'] . " AND threadid = " . $threadid ."
				AND sendstatus = 1");
		...
		// compare with the user behaviour
		$totaleffect = $vbulletin->db->query_read_slave("SELECT COUNT(*) AS total_effect
			FROM " . TABLE_PREFIX . "`user` 
			LEFT " . TABLE_PREFIX . " JOIN push_notification_send AS pns ON user.`userid` = pns.userid
		...
		while($totaleffect = $vbulletin->db->fetch_array($totaleffect))
		{
			...
			if($ifexists['dataexist'] != 0)
			{
				if($ifexists['dateline'] < intval($lastposttime['dateline']))
				{
					...
					sendNotificationBySpecificUser('', $threadlink, "$pagetext_orig");
				}
			...
			else
			{
				...
				// $hasbrowserid = $vbulletin->db->query_read("SELECT browserid FROM " . TABLE_PREFIX . " push_notification WHERE userid = " . $vbulletin->userinfo['userid'] . " AND browserid = " . $brid  . "
				// 	");

				// if($hasbrowserid['browserid'] != null)
				// {

				// }else {
				// 	$vbulletin->db->query_write("
				// 		INSERT INTO " . TABLE_PREFIX . " push_notification
				// 		(
				// 			browserid,
				// 			userid,
				// 			dateline
				...
				sendNotificationBySpecificUser('', '', "Web Push Notification is enabled for this thread || threadid: $threadid");
			}
		}
	}
}
...
/**
* more info visit https://app.onesignal.com/
* @param string which generated by onesignal
* @param string browser id must be unique | generated by onesignal api
* @param string heading of your PN
* @param string body of your PN
*/
function sendNotificationBySpecificUser($browserid = '', $url = '', $message = ''){

	global $vbulletin;

	$wpn = $vbulletin->db->query_first("SELECT * FROM " . TABLE_PREFIX . " push_notification_key ORDER BY id DESC LIMIT 1");
	
	$app_id = $wpn['appId'];
	$browserid = ($browserid != '') ? $browserid : $_COOKIE['os_uid'];
	$heading = $vbulletin->options['bburl'];
	$url = $url;
    ...
    $fields = array(
        'app_id' => $app_id,
        'include_player_ids' => array($browserid),
        'contents' => $content,
        'headings' => $headings,
        'url'  => $url
    );
...
function fetch_quotable_posts($quote_postids, $threadid, &$unquoted_posts, &$quoted_post_ids, $limit_thread = 'only', $unhtmlspecialchars = false)
{
	...
	foreach ($quote_postids AS $quote_postid)
	{
		...
		$pagetext = $quote_post['pagetext'];
		$namequote =  get_string_between($pagetext,'[QUOTE="','"]');
		if($namequote){ $quote_name = $namequote ; }else{$quote_name = $quote_post['username']; }
		$originalposter = fetch_quote_username($quote_name . ";$quote_post[postid]");
